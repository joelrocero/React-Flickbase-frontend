...
const Articles = (props) => {
  ...
  const [searchValues, setSearchValues] = useReducer((state, newState) => ({ ...state, ...newState }), { value: '', memory: '' })
  let limit = 5
      ...
      <div className='articles_table'>
        <ButtonToolbar className='mb-3'>
          ...
          <form onSubmit={triggerSearch}>
            <InputGroup>
              ...
              <FormControl type='text' placeholder='Example' value={searchValues.value} onChange={(e) => setSearchValues({ value: e.target.value })} />
            </InputGroup>
          </form>
        </ButtonToolbar>
        ...
        {loading ? (
          ...
          <>
            <div>
              {searchValues.memory !== '' ? (
                <p>
                  ...
                  <span style={{ color: 'blue', cursor: 'pointer' }} onClick={() => resetSearch()}>
                    RESET SEARCH
                  </span>
                </p>
              ) : null}
            ...
            <PaginationComponent arts={arts} prev={(page) => goToPrevPage(page)} next={(page) => goToNextPage(page)} handleShow={(id) => handleShow(id)} handleStatusChange={(status, id) => handleStatusChange(status, id)} editArtsAction={(id) => editArtsAction(id)} />
          </>
        )}
