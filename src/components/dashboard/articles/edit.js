...
import { TextField, Button, Divider, Chip, Paper, InputBase, IconButton, Select, MenuItem, FormControl, FormHelperText } from '@material-ui/core'
    ...
    <AdminLayout section='Add article'>
      {isSubmitting ? (
        ...
        <form className='mt-3 article_form' onSubmit={formik.handleSubmit}>
          ...
          <div className='form-group'>
            <WYSIWYG setEditorState={(state) => handleEditorState(state)} setEditorBlur={(blur) => handleEditorBlur(blur)} editContent={editContent} />

            {formik.errors.content && editorBlur ? <FormHelperText error={true}>{formik.errors.content}</FormHelperText> : null}

            <TextField type='hidden' name='content' {...formik.getFieldProps('content')} />
          </div>

          <div className='form-group'>
            <TextField style={{ width: '100%' }} name='excerpt' label='Enter an excerpt' variant='outlined' {...formik.getFieldProps('excerpt')} {...errorHelper(formik, 'excerpt')} multiline rows={4} />
          </div>
            ...
            <FieldArray
              ...
              render={(arrayhelpers) => (
                <div>
                  ...
                  {formik.errors.actors && formik.touched.actors ? <FormHelperText error={true}>{formik.errors.actors}</FormHelperText> : null}
          ...
          <FormControl variant='outlined'>
            ...
            <Select name='category' {...formik.getFieldProps('category')} error={formik.errors.category && formik.touched.category ? true : false}>
              <MenuItem value=''>
                <em>None</em>
              </MenuItem>
              {articles.categories
            ...
            {formik.errors.category && formik.touched.category ? <FormHelperText error={true}>{formik.errors.category}</FormHelperText> : null}
          </FormControl>
          ...
          <FormControl variant='outlined'>
            ...
            <Select name='status' {...formik.getFieldProps('status')} error={formik.errors.status && formik.touched.status ? true : false}>
              <MenuItem value=''>
                <em>None</em>
              </MenuItem>
            ...
            {formik.errors.status && formik.touched.status ? <FormHelperText error={true}>{formik.errors.status}</FormHelperText> : null}
          </FormControl>
