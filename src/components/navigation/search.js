...
const NavSearch = (props) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      keywords: Yup.string().required('Sorry this is required').min(3, 'Just 3 ? come on !!!'),
    }),
    onSubmit: (values) => {
  ...
  return (
    <>
      <form style={{ margin: '20px' }} onSubmit={formik.handleSubmit}>
        <TextField label='Search movie' variant='outlined' name='keywords' {...formik.getFieldProps('keywords')} {...errorHelper(formik, 'keywords')} />
      </form>
    </>
  )
}
